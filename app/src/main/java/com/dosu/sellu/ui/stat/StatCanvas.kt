...
class StatCanvas(
    ...
    fun drawMoney(stats: List<Stat>){
        this.stats = stats
        isMoney = true
        invalidate()
    }

    fun drawCounts(stats: List<Stat>){
        isMoney = false
        this.stats = stats
    }
    ...
    private val barCountPaint = Paint(barMoneyPaint).apply {
        color = ContextCompat.getColor(context, R.color.count)
    }
    ...
    private fun initValues(){
        ...
        barWidthUnit = if(isMoney) {
            val maxCash = stats.maxOf { stat -> stat.money }
            barMaxWidth / maxCash.toFloat()
        }else{
            val maxCount = stats.maxOf { stat -> stat.count }
            barMaxWidth / maxCount.toFloat()
        }
    ...
    private fun drawBars(canvas: Canvas){
        canvas.apply {
            if(isMoney) for(i in stats.indices){
            ...
            else for (i in stats.indices){
                drawRect(textWidth, y(i), textWidth+barCount(i), y(i)+barHeight, barCountPaint)
            }
        }
    }
    ...
    private fun barCount(i: Int): Float{
        return (barWidthUnit*stats[i].count)
    }
